        ...
        <x-core.form-wizard.content target="tab-style-2-2">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                    ...
                    <x-core.forms.input
                        :model="$guideQuest"
                        label="Required (Player) Reincarnations:"
                        modelKey="required_reincarnation_amount"
                        name="required_reincarnation_amount"
                    />
                    ...
                    <div class="my-6 block border-b-2 border-b-gray-300 md:hidden dark:border-b-gray-600"></div>
                ...
                <div>
                    ...
                    <x-core.forms.input
                        :model="$guideQuest"
                        label="Required Delve Survival Hour(s):"
                        modelKey="required_delve_survival_time"
                        name="required_delve_survival_time"
                    />
                    <x-core.forms.input
                        :model="$guideQuest"
                        label="Required Delve Pack Size:"
                        modelKey="required_delve_pack_size"
                        name="required_delve_pack_size"
                    />
                    ...
                    <x-core.forms.key-value-select
                        :model="$guideQuest"
                        label="Required Batch Crafting Type:"
                        modelKey="required_batch_crafting_type"
                        name="required_batch_crafting_type"
                    ...
                    <x-core.forms.input
                        :model="$guideQuest"
                        label="Required Batch Crafting Hour(s):"
                        modelKey="required_batch_crafting_hours"
                        name="required_batch_crafting_hours"
                        helpText="Counts when the player runs the selected batch crafting type in experience mode for at least this many hours. It remains valid if the batch finishes, times out, or is cancelled after the required time is met."
                    />
                    ...
                    <p class="mb-3 text-sm text-gray-600 dark:text-gray-400">
                        Requires the player to have this many matching items. Crafted equipment is checked in normal
                        inventory. Alchemy items are checked in the player’s alchemy bag. Crafted equipment marked as
                        enchanted must have both a prefix and a suffix. One enchant is not enough. Alchemy items cannot
                        be enchanted. These items are consumed when the guide quest is handed in.
                    </p>
                        ...
                        <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-4">
                            <div>
                                <label
                                    ...
                                    for="required_batch_crafted_items_{{ $batchCraftedItemIndex }}_source"
                                >Source</label>
                                <select
                                    class="focus:border-primary-500 focus:ring-primary-500 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:ring-2 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
                                ...
                                >
                                    <option
                                        value="inventory"
                                        @selected(($batchCraftedItemRow['source'] ?? 'inventory') === 'inventory')
                                    >
                                        Crafted Equipment
                                    </option>
                                    <option
                                        value="alchemy_bag"
                                        @selected(($batchCraftedItemRow['source'] ?? 'inventory') === 'alchemy_bag')
                                    >
                                        Alchemy Bag
                                    </option>
                                </select>
                            ...
                            <div>
                                <label
                                    class="mb-2 block text-sm font-semibold text-gray-900 dark:text-gray-100"
                                    for="required_batch_crafted_items_{{ $batchCraftedItemIndex }}_item_id"
                                >Item</label>
                                <select
                                    class="focus:border-primary-500 focus:ring-primary-500 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:ring-2 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
                                ...
                                >
                                    ...
                                    @foreach ($batchCraftedItemOptions as $itemId => $itemLabel)
                                        <option
                                            value="{{ $itemId }}"
                                            @selected((string) ($batchCraftedItemRow['item_id'] ?? '') === (string) $itemId)
                                        >
                                            {{ $itemLabel }}
                                        </option>
                                    @endforeach
                            ...
                            <div>
                                <label
                                    class="mb-2 block text-sm font-semibold text-gray-900 dark:text-gray-100"
                                    for="required_batch_crafted_items_{{ $batchCraftedItemIndex }}_amount"
                                >Amount</label>
                                <input
                                    class="focus:border-primary-500 focus:ring-primary-500 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm placeholder:text-gray-500 focus:ring-2 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-400"
                            ...
                            <div>
                                <label
                                    class="mb-2 block text-sm font-semibold text-gray-900 dark:text-gray-100"
                                    for="required_batch_crafted_items_{{ $batchCraftedItemIndex }}_must_be_enchanted"
                                >Must Be Enchanted</label>
                                <select
                                    class="focus:border-primary-500 focus:ring-primary-500 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:ring-2 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
                                ...
                                >
                                    <option
                                        value="0"
                                        @selected(! filter_var($batchCraftedItemRow['must_be_enchanted'] ?? false, FILTER_VALIDATE_BOOLEAN))
                                    >
                                        No
                                    </option>
                                    <option
