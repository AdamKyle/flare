    ...
    <x-core.layout.info-container>
        ...
        >
            @if (auth()->user()->hasRole('Admin'))
                <x-core.buttons.link-buttons.primary-button href="{{ route('admin.guide-quests.edit', ['guideQuest' => $guideQuest->id]) }}">
                    Edit Quest
        ...
        <x-core.cards.card-with-title title="Info">
            ...
            <x-core.dl.dl class="my-4">
                ...
                <x-core.dl.dd>
                    {{ is_null($guideQuest->xp_reward) ? 0 : number_format($guideQuest->xp_reward) }}</x-core.dl.dd>
                <x-core.dl.dt>Gold Reward</x-core.dl.dt>
                <x-core.dl.dd>
                    {{ is_null($guideQuest->gold_reward) ? 0 : number_format($guideQuest->gold_reward) }}</x-core.dl.dd>
                <x-core.dl.dt>Gold Dust Reward</x-core.dl.dt>
                <x-core.dl.dd>
                    {{ is_null($guideQuest->gold_dust_reward) ? 0 : number_format($guideQuest->gold_dust_reward) }}</x-core.dl.dd>
                <x-core.dl.dt>Shards Reward</x-core.dl.dt>
                <x-core.dl.dd>
                    {{ is_null($guideQuest->shards_reward) ? 0 : number_format($guideQuest->shards_reward) }}</x-core.dl.dd>
            </x-core.dl.dl>
            ...
            @if (! is_null($guideQuest->eventType()) || ! is_null($guideQuest->unlock_at_level))
                <div class="my-4">
                    <x-core.alerts.info-alert title="Event Specific Guide Quest!">
                        <p>
                            This Guide quest is only available when it unlocks at a specific level, is used for an event
                            or both.
                        </p>
                        <p class="my-2">
                            These types of quests interrupt what the player was doing to introduce them to new features.
                        </p>
                        ...
                        <x-core.dl.dl>
                            ...
                            <x-core.dl.dd>
                                {{ is_null($guideQuest->parent_id) ? 'N/A' : $guideQuest->parent_quest_name }}</x-core.dl.dd>
                        </x-core.dl.dl>
