    ...
    <div class="mt-10 mb-10 w-full lg:w-3/5 m-auto">
        ...
        <x-core.cards.card>
            <form action="{{is_null($skill) ? route('passive.skill.store') : route('passive.skill.update', ['passiveSkill' => $skill->id])}}" method="POST">
                  ...
                  <x-core.forms.input :model="$skill" label="Resource Request Travel Time Reduction % (Optional):" modelKey="resource_request_time_reduction" name="resource_request_time_reduction" />
                  ...
                  <x-core.forms.check-box :model="$skill" label="Is Locked?" modelKey="is_locked" name="is_locked" />
                  <x-core.forms.check-box :model="$skill" label="Is Parent Skill?" modelKey="is_parent" name="is_parent" />


                    @if (is_null($skill))
                        <x-core.buttons.primary-button type="submit">Create Passive</x-core.buttons.primary-button>
                    @else
                        <x-core.buttons.primary-button type="submit">Update Passive</x-core.buttons.primary-button>
                    @endif
            </form>
        </x-core.cards.card>
    </div>
