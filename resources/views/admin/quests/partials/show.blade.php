<div class="grid lg:grid-cols-2 gap-4">
    <div>
        <p class="mt-3 mb-3">
            This quest belongs to an NPC, whom you must be on the same place as
            to complete.
        </p>
        <p class="mb-3">
            To complete all a quest all you have to do is on the same plane,
            then click Quests tab, click the quest, click complete.
        </p>
        <p class="mb-3">
            You must have the required items, currencies and/or faction points
            needed.
        </p>
        ...
        @if (! is_null($quest->raid_id))
            <x-core.alerts.info-alert title="ATTN!">
                <p>
                    This quest chain is only available during the:
                    {{ $quest->raid->name }} Raid.
                </p>
        ...
        @if (! is_null($quest->only_for_event))
            @if ($quest->eventType()->isWinterEvent())
                <x-core.alerts.info-alert title="ATTN!">
                    <p>
                        This quest chain is only available during the Winter
                        Event
                    </p>
                </x-core.alerts.info-alert>
            @endif
        ...
        <dl>
            ...
            @if ($quest->npc->must_be_at_same_location)
                <dt>Npc X/Y:</dt>
                <dd>
                    {{ $quest->npc->x_position }}/{{ $quest->npc->y_position }}
                    <strong>{{ $quest->npc->gameMap->name }}</strong>
                    (You will be moved to this location upon handing in the
                    quest.)
                </dd>
            @endif
