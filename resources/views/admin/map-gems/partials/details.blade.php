@php
    ...
    $craftingSkillNames = \App\Flare\Models\GameSkill::whereIn('id', $gameMapGemParamters->crafting_skill_ids ?? [])
        ->orderBy('name')
        ->pluck('name')
        ->implode(', ');
    $mapRoute = auth()->check() && auth()->user()->hasRole('Admin')
        ? route('map', ['gameMap' => $gameMapGemParamters->gameMap])
        : route('info.page.map', ['map' => $gameMapGemParamters->gameMap]);
    $sections = [
        'Player Rewards' => [
            'info_title' => 'Player reward modifiers',
            'info_content' => 'Stored percentage ranges that will later affect character progression, class progression, faction rewards, passive training, crafting skill gains, and any map-only character power reduction.',
            'fields' => [
...
<div class="space-y-6 text-gray-700 dark:text-gray-300">
    <div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="whitespace-pre-line text-sm leading-6 text-gray-700 dark:text-gray-300">{{ filled($gameMapGemParamters->description) ? $gameMapGemParamters->description : 'N/A' }}</p>
    </div>
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <dl class="grid grid-cols-1 gap-x-8 gap-y-4 lg:grid-cols-[minmax(10rem,16rem)_minmax(0,1fr)_minmax(10rem,16rem)_minmax(0,1fr)]">
                ...
                <dd class="text-sm leading-6 text-gray-700 dark:text-gray-300">
                    <a href="{{ $mapRoute }}">{{ $gameMapGemParamters->gameMap->name }}</a>
                </dd>
                @if(! is_null($gameMapGemParamters->monster_atonement) && isset($atonementNames[$gameMapGemParamters->monster_atonement]))
                    <dt class="text-sm font-semibold leading-6 text-gray-900 dark:text-gray-100">Monster Atonement</dt>
                    <dd class="text-sm leading-6 text-gray-700 dark:text-gray-300">{{ $atonementNames[$gameMapGemParamters->monster_atonement] }}</dd>
                @endif
                @if(filled($gameMapGemParamters->monster_atonement_range))
                    <dt class="text-sm font-semibold leading-6 text-gray-900 dark:text-gray-100">Monster Atonement Range</dt>
                    <dd class="text-sm leading-6 text-gray-700 dark:text-gray-300">+{{ $gameMapGemParamters->monster_atonement_range }}%</dd>
                @endif
    ...
    @foreach($sections as $sectionTitle => $section)
        @php
            $visibleRows = collect($section['fields'])
                ->map(function (array $fieldConfig, string $field) use ($gameMapGemParamters): ?array {
                    [$label, $colorClass] = $fieldConfig;
                    if (! filled($gameMapGemParamters->{$field})) {
                        return null;
                    }
                    return ['label' => $label, 'value' => '+'.$gameMapGemParamters->{$field}.'%', 'class' => $colorClass];
                })
                ->filter();
        @endphp
