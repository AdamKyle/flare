<x-core.cards.card-with-title title="Monster Stats" buttons="false">
    <div class="grid gap-4 md:grid-cols-2">
        ...
        <div class="my-3 block border-b-2 border-b-gray-300 md:hidden dark:border-b-gray-600"></div>
        <div>
            ...
            <x-core.dl.dl>
                ...
                <x-core.dl.dd>
                    {{ number_format(explode('-', $monster->health_range)[0]) }} - {{ number_format(explode('-', $monster->health_range)[1]) }}
                </x-core.dl.dd>
                <x-core.dl.dt>Attack Range</x-core.dl.dt>
                <x-core.dl.dd>
                    {{ number_format(explode('-', $monster->attack_range)[0]) }} - {{ number_format(explode('-', $monster->attack_range)[1]) }}
                </x-core.dl.dd>
...
@if (! is_null($monster->fire_atonement) && ! is_null($monster->ice_atonement) && ! is_null($monster->water_atonement))
    <x-core.cards.card-with-title title="Elemental Atunement">
        <p class="mb-3">
            ...
            work for characters. The higher the value, the more attuned to thay element the critter is, which means tier
            elemental attack will be of that type. If your elemental atunement is the oppisite, for example Water is
            oppisite to Fire, you will only take half the damage, If your element is weak against their, ie: They are
            fire, you are ice, you will take twice the amount of damage.
        </p>
