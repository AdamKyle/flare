    ...
    <div class="pb-5">
        ...
        @if ($monster->is_raid_boss)
            <x-core.alerts.info-alert title="ATTN!">
                This is a raid boss and cannot be encountered by traditional
                means.
        ...
        @if ($monster->is_raid_monster)
            <x-core.alerts.info-alert title="ATTN!">
                This is a raid monster and cannot be encountered by traditional
                means.
            </x-core.alerts.info-alert>
        @endif

        @include(
            'admin.monsters.partials.details',
            [
                'monster' => $monster,
                'quest' => $quest,
                'canEdit' => true,
        ...
        @if (! is_null($monster->quest_item_id))
            <x-core.cards.card-with-title
                title="Quest Item Details"
                buttons="false"
            >
                <h3 class="mt-4 mb-4">
                    <x-item-display-color :item="$monster->questItem" />
                </h3>
                <p class="mb-4">
                    Quest items are used automatically upon gaining them.
                </p>
                <p class="mb-4">
                ...
                @if (! is_null($quest))
                    <p class="mb-4">
                        Used in:
                        <a
                            href="{{ route('info.page.quest', ['quest' => $quest->id]) }}"
                        >
                            {{ $quest->name }}
                        </a>
                    </p>
