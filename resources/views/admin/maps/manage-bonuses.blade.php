...
@section('content')
    <x-core.layout.info-container>
        ...
        >
            <form
                class="mt-4"
                action="{{ route('add.map.bonuses', ['gameMap' => $gameMap]) }}"
                method="POST"
            ...
            >
                ...
                <x-core.forms.input
                    ...
                    modelKey="drop_chance_bonus"
                    name="drop_chance_bonus"
                    type="number"
                />
                <x-core.forms.input
                    :model="$gameMap"
                    label="Enemy Stat increase (%):"
                    modelKey="enemy_stat_bonus"
                    name="enemy_stat_bonus"
                    type="number"
                />
                <x-core.forms.input
                    :model="$gameMap"
                    label="Character Attack Reduction (%):"
                    modelKey="character_attack_reduction"
                    name="character_attack_reduction"
                    type="number"
                />
                <x-core.forms.collection-select
                    :model="$gameMap"
                    label="Required Location:"
                    modelKey="required_location_id"
                    name="required_location_id"
                    value="id"
                    key="name"
                    :options="$locations"
                />
                ...
                <x-core.buttons.primary-button type="submit"> Submit </x-core.buttons.primary-button>
            </form>
        </x-core.cards.card-with-title>
    </x-core.layout.info-container>
@endsection
