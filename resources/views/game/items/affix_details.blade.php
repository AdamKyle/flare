<x-core.modals.modal id="affix-details-{{ $itemAffix->id }}" title="{{ $itemAffix->name }}" largeModal="true">
    <p class="mt-4 mb-4">{{ $itemAffix->description }}</p>
    <div class="my-3 border-b-2 border-b-gray-300 dark:border-b-gray-600"></div>

    <div class="{{ $itemAffix->damage !== 0 || $itemAffix->reduces_enemy_stats || ! is_null($itemAffix->steal_life_amount) || $itemAffix->entranced_chance > 0 || $itemAffix->devouring_light > 0 || $itemAffix->resistance_reduction > 0 ? 'grid max-h-[350px] gap-2 overflow-y-auto md:grid-cols-2' : ' max-h-[350px] overflow-y-auto' }}">
        <div>
        ...
        <div>
            ...
            @if ($itemAffix->resistance_reduction > 0)
                <h3 class="text-sky-600 dark:text-sky-500">Enemy Resistance Reduction</h3>
                <div class="my-3 border-b-2 border-b-gray-300 dark:border-b-gray-600"></div>
                <dl>
