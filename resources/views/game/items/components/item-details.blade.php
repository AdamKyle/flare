<div @if ($item->type === 'quest') class="grid gap-4 md:grid-cols-2" @else class="" @endif>
    <div>
        <x-core.cards.card-with-title title="Details" buttons="false">
            <p class="mb-4 text-sky-600 italic dark:text-sky-400">{{ nl2br($item->description) }}</p>
        ...
        @if (! is_null($item->itemSkill))
            <x-core.cards.card-with-title title="Item Skills">
                <p class="my-4">
                    This item has skills attached to it. below is the parent skill, from there, you will see other child
                    skills this item has to offer.
                </p>

                <p class="my-4">
                    Items with item skills cannot be sold on the market and cannot be sold. They are bound to your
                    character and there skills effects only take place whe equipped.
                </p>
