...
@section('content')
    <x-core.layout.info-container>
        <x-core.cards.card-with-title title="Survey Stats" buttons="true" backUrl="{{ route('welcome') }}">
            @if (!$surveyExists)
                <p class="my-4">
                    There is no survey statistics as of yet. Check back after
                    the event has finished :D
                </p>
            @else
                <h1>{{ $survey['title'] }}</h1>
                ...
                <p class="my-4">
                    <strong>
                        Total player percentage who completed the survey:
                    </strong>
                ...
                <x-core.alerts.info-alert title="Quick note">
                    <p>
                        Scroll down to see the responses of the server. At the
                        bottom of the survey you will find The Creators response
                        to the survey results.
                    </p>
                    <p class="my-2">
                        %'s beside the questions indicates how many people chose
                        that option for said question.
                    </p>
                            ...
                            @foreach ($section['input_types'] as $field)
                                <div>
                                    ...
                                    @elseif ($field['type'] === 'checkbox')
                                        ...
                                        @foreach ($field['options'] as $index => $option)
                                            <div class="flex items-center mt-1">
                                                <input disabled id="checkbox-{{ $index }}" type="checkbox"
                                    ...
                                    @elseif ($field['type'] === 'radio')
                                        ...
                                        @foreach ($field['options'] as $index => $option)
                                            <div class="flex items-center mt-1">
                                                <input disabled id="radio-{{ $index }}" type="radio"
                                                    name="{{ $field['label'] }}"
