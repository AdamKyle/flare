...
<x-core.cards.feature-card>
  <x-slot:icon>
    <i
      class="ra {{ $icon }} text-primary-600 relative top-2 right-2 md:top-4 md:right-4"
    ></i>
  </x-slot>
  <x-slot:title>
  ...
  <p class="mb-4 text-sm md:text-base">
    @if ($eventRunning->getTitleOfEvent() === 'Weekly Celestials')
        Join the Celestials battle! Take them down and gain valuable currencies!
        Celestials spawn with an 80% chance. Just move around the map to engage!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'Weekly Currency Drops')
        Get a boost to currencies like Gold Dust, Crystal Shards, and Copper
        Coins! Great for advanced crafting. This boon also applies to Slots and
        special locations!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'Weekly Faction Loyalty Event')
        Get a boost to working on your Faction Loyalties. Gain 2 points instead
        of one. When the fame levels up, the requirements for the next level are
        halved!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'The Winter Event')
        Winter event is here! Enter the temporary map for powerful loot and epic
        adventures. Settle kingdoms to win a full set of Corrupted Ice Gear!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'Delusional Memories Event')
        Dive into Delusional Memories! Explore the temporary map for powerful
        loot and epic adventures. Settle kingdoms to win a full set of
        Delusional Silver Gear!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'The Jester of Time')
        Join the Jester of Time raid! Head to Southren Port in Delusional
        Memories and band together! The player who kills him gets a new
        Ancestral item!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'The Smugglers Are Back!')
        Join the Smugglers are Back raid! Head to Smugglers Port on Surface and
    ...
    @if ($eventRunning->getTitleOfEvent() === 'The Frozen King')
        ...
        Ice Plane and band together! The player who kills him gets a new
        Ancestral item!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'Corrupted Bishop')
        Join the Corrupted Bishop raid! head down to the Delusional Memories
        Federation Controlled Town to band together! The Player who kills him
        gets a new Ancestral item!
    @endif

    @if ($eventRunning->getTitleOfEvent() === 'Tlessa\'s Feedback Event')
        Tlessa wants your feedback! We offer increased XP, 80% drop rate for
        everyone! and a Mythical item for taking a survey!
    @endif
  </p>
