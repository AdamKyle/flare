...
@if ($isTailwind)
  <div
    class="flex-cols flex"
    x-data="newBooleanFilter(
              '{{ $filter->getKey() }}',
              '{{ $tableName }}',
              '{{ $defaultValue }}',
            )"
  >
    ...
    >
      <span
        :class="(value == 1 || value == true) ? 'translate-x-[18px]' : 'translate-x-0.5'"
        class="{{ $filterInputAttributes['blobColor'] }} h-5 w-5 rounded-full shadow-md duration-200 ease-in-out"
      ></span>
    </button>
    <template x-if="value == 1 || value == true">
      <button
        @click="toggleStatusWithReset"
        type="button"
        class="ml-1 inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-indigo-400 hover:bg-indigo-200 hover:text-indigo-500 focus:bg-indigo-500 focus:text-white focus:outline-none"
      >
        <span class="sr-only">
