...
@if ($this->bulkActionsAreEnabled() && $this->hasBulkActions())
  ...
  >
    <x-livewire-tables::table.td.plain :colspan="$colspan">
      <template
        x-if="selectedItems.length == paginationTotalItemCount || selectAllStatus"
      >
          ...
          <button
            ...
            {{
              $this->getBulkActionsRowButtonAttributesBag->class([
                'ml-1 text-sm leading-5 font-medium underline transition duration-150 ease-in-out focus:underline focus:outline-none' => $isTailwind && ($this->getBulkActionsRowButtonAttributes['default-styling'] ?? true),
