...
@if ($this->bulkActionsAreEnabled() && $this->hasBulkActions())
  ...
  >
        ...
        $watch(
          ...
          (value) =>
            (indeterminateCheckbox =
              value.length > 0 && value.length < paginationTotalItemCount),
        )
      "
      x-cloak
    ...
    >
      <input
        x-init="
          $watch('indeterminateCheckbox', (value) => ($el.indeterminate = value))
          $watch('selectedItems', (value) => (newSelectCount = value.length))
        "
        x-on:click="
          if (selectedItems.length == paginationTotalItemCount) {
        ...
        {{
          $attributes
            ->merge($bulkActionsThCheckboxAttributes)
            ->class([
