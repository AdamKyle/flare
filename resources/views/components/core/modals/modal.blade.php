...
<div x-data="{ modalIsOpen: {{ $open ? "true" : "false" }} }" {{ $attributes->merge(["class" => ""]) }}>
    @isset($trigger)
        <span class="block w-full" x-on:click="modalIsOpen = true"> {{ $trigger }} </span>
    @endisset
    ...
    >
        ...
        >
            ...
            <div class="max-h-[350px] overflow-x-auto px-4 py-6 text-neutral-800 md:max-h-[500px] dark:text-neutral-100">
                {{ $slot }}
            </div>

            @isset($footer)
                <div class="flex flex-col-reverse justify-between gap-2 border-t border-neutral-200 bg-neutral-50/60 p-4 sm:flex-row sm:items-center md:justify-end dark:border-neutral-700 dark:bg-neutral-900/40">
                    {{ $footer }}
                </div>
            @endisset
        </div>
