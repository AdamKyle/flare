...
<div class="{{ $css }} overflow-hidden rounded-2xl bg-white text-gray-800 shadow-xl dark:bg-gray-800 dark:text-gray-200 mb-4">
    <div class="flex items-center justify-between border-b border-gray-200 p-8 dark:border-gray-700">
        @if ($buttons != 'false')
            @if (! is_null($route))
            ...
            @else
                <h2 class="text-3xl font-medium text-gray-900 dark:text-gray-100">{!! $title !!}</h2>
            @endif
    ...
    <div class="p-8 text-gray-700 dark:text-gray-300">{{ $slot }}</div>
</div>
