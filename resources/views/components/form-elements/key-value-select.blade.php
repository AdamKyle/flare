...
<div
    ...
    class="relative mb-5"
>
    <label for="combobox-{{ $name }}" class="mb-2 block text-sm font-medium text-gray-600 dark:text-gray-300">
        {{ $label }}
    </label>
    ...
    <input
        ...
        @keydown.arrow-down.prevent="
            open = true;
            $refs.list.querySelector('li')?.focus();
        "
        role="combobox"
    ...
    >
        ...
        <li x-show="filtered.length === 0" class="px-3 py-2 text-gray-500 dark:text-gray-400">No results</li>
    </ul>

    @error($name)
        <p id="{{ $errorId }}" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
    @enderror
</div>
