@php
    ...
    $craftingSkillNames = \App\Flare\Models\GameSkill::whereIn('id', $gameLocationGemParamter->crafting_skill_ids ?? [])
        ->orderBy('name')
        ->pluck('name')
        ->implode(', ');
    $locationRoute = auth()->check() && auth()->user()->hasRole('Admin')
        ? route('locations.location', ['location' => $gameLocationGemParamter->location])
        : route('info.page.location', ['location' => $gameLocationGemParamter->location]);
    $mapRoute = auth()->check() && auth()->user()->hasRole('Admin')
        ? route('map', ['gameMap' => $gameLocationGemParamter->location->map])
        : route('info.page.map', ['map' => $gameLocationGemParamter->location->map]);
    $sections = [
        'Player Rewards' => [
