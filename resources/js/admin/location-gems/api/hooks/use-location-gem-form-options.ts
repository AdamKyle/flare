...
import UseLocationGemFormOptionsDefinition from './definitions/use-location-gem-form-options-definition';
...
export const useLocationGemFormOptions =
  (): UseLocationGemFormOptionsDefinition => {
    const { apiHandler, getUrl } = useApiHandler();

    const [formOptions, setFormOptions] =
      useState<LocationGemFormOptionsDefinition | null>(null);
      ...
      const fetchFormOptions = async () => {
        try {
          const result = await apiHandler.get<
            LocationGemFormOptionsDefinition,
            Record<string, never>
          >(getUrl(LocationGemApiUrls.OPTIONS), {
          ...
          setError({ message: LocationGemApiMessages.LoadFormOptions });
        } finally {
          if (requestGenerationRef.current === requestGeneration) {
            setLoading(false);
          }
