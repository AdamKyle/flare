import React from "react";
import { ActiveExplorer } from "../types/exploration-monitoring";
import ActiveExplorersTableProps from "../types/active-explorers-table-props";
import MonitoringCard from "./monitoring-card";

export default class ActiveExplorersTable extends React.Component<ActiveExplorersTableProps> {
    renderRows() {
        return this.props.explorers.map((explorer: ActiveExplorer) => (
            <tr
                className="border-t dark:border-gray-700"
                key={explorer.character_id}
            >
                <td className="p-2 font-medium">
                    {explorer.character_name ?? "—"}
                </td>
                <td className="p-2">{explorer.monster_name ?? "—"}</td>
                <td className="p-2">{explorer.attack_type}</td>
                <td className="p-2">{explorer.started_at ?? "—"}</td>
                <td className="p-2">{explorer.completed_at ?? "—"}</td>
            </tr>
        ));
    }

    render() {
        return (
            <MonitoringCard
                title="Currently Exploring"
                description="Characters with an active exploration automation."
            >
                {this.props.explorers.length === 0 ? (
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                        No characters are currently exploring.
                    </p>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[600px] text-left text-sm">
                            <thead>
                                <tr className="border-b dark:border-gray-700">
                                    <th scope="col" className="p-2">
                                        Character
                                    </th>
                                    <th scope="col" className="p-2">
                                        Monster
                                    </th>
                                    <th scope="col" className="p-2">
                                        Attack type
                                    </th>
                                    <th scope="col" className="p-2">
                                        Started at
                                    </th>
                                    <th scope="col" className="p-2">
                                        Ends at
                                    </th>
                                </tr>
                            </thead>
                            <tbody>{this.renderRows()}</tbody>
                        </table>
                    </div>
                )}
            </MonitoringCard>
        );
    }
}
