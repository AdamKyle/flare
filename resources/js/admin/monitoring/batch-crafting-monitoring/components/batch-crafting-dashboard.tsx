          ...
          {active.map((row) => (
            ...
            >
              ...
              <dl className="mt-2 grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <dt className="text-gray-500 dark:text-gray-400">Crafted</dt>
  ...
  const renderRecentRunsMobile = () => {
    ...
    return runs.data.map((row) => (
      <div
        ...
        className="rounded border border-gray-200 p-3 text-sm dark:border-gray-700"
      >
        <div className="font-semibold">{row.character?.name ?? '—'}</div>
        <dl className="mt-2 grid grid-cols-2 gap-2 text-xs">
          <div>
            <dt className="text-gray-500 dark:text-gray-400">Type</dt>
            <dd>{row.batch_type}</dd>
          ...
          <div>
            ...
            <dd>{humanizeBatchCraftingStatus(row)}</dd>
          </div>
          <div>
            <dt className="text-gray-500 dark:text-gray-400">Crafted</dt>
            <dd>{row.crafted_count}</dd>
  ...
  const renderRecentRunsDesktop = () => {
    ...
    return runs.data.map((row) => (
      <tr
        ...
        className="border-b border-gray-100 dark:border-gray-800"
      >
        <td className="py-2 pr-4">{row.character?.name ?? '—'}</td>
        <td className="py-2 pr-4">{row.batch_type}</td>
        <td className="py-2 pr-4">{row.disposition}</td>
        <td className="py-2 pr-4 capitalize">
          {humanizeBatchCraftingStatus(row)}
        </td>
        <td className="py-2 pr-4">{row.started_at ?? '—'}</td>
        <td className="py-2 pr-4">{row.crafted_count}</td>
        <td className="py-2">{row.failed_count}</td>
    ...
    <div className="space-y-5 pb-16 text-gray-900 dark:text-gray-100">
      ...
      <div id={runsTableElementId}>
        <MonitorCard>
          ...
          <div className="grid gap-3 md:hidden">{renderRecentRunsMobile()}</div>
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
              ...
              <tbody>{renderRecentRunsDesktop()}</tbody>
            </table>
          </div>
