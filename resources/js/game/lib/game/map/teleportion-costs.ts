 ...
 * }
 */
export const fetchCost = (xPosition: number, yPosition: number, characterPosition: {x: number, y: number}, currencies: {gold: number, gold_dust: number, shards: number} | null): {
    can_afford: boolean,
    distance: number,
    cost: number,
    time_out: number,
