...
export default class TripleAttack extends BattleBase {
    ...
    public handleAttack(character: CharacterType, attackType: AttackType, extraAction: ExtraActionType, monsterCurrentHealth: number): number {
        ...
        if (extraAction.type === SpecialType.RANGER_TRIPLE_ATTACK && extraAction.has_item) {
    ...
    public doDamage(monsterCurrentHealth: number, damage: number): number {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            this.addMessage('You hit for (weapon - triple attack) ' + formatNumber(damage), 'player-action');
        }
