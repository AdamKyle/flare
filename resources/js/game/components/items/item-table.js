...
var ItemTable = (function (_super) {
    ...
    ItemTable.prototype.setSelectedFilterType = function (data) {
        ...
        var filteredItems = this.props.items.filter(function (item) { return item.type === data.value; });
    ...
    ItemTable.prototype.render = function () {
        var _a;
        return (React.createElement(React.Fragment, null, this.props.item_to_view !== null ? (React.createElement("div", null,
            React.createElement(PrimaryOutlineButton, { button_label: this.props.close_view_item_label, on_click: this.props.close_view_item_action, additional_css: "my-3" }),
            React.createElement(BasicCard, null,
                React.createElement(Item, { item: this.props.item_to_view })))) : (React.createElement(BasicCard, { additionalClasses: "my-4" },
            React.createElement(InfoAlert, { additional_css: "my-3" },
                React.createElement("p", null, "Click the name of the item to get more info about it's stats.")),
            React.createElement("div", { className: "md:w-3/5 w-full my-4" },
                React.createElement("div", { className: "grid md:grid-cols-3 gap-4 my-4" },
                    React.createElement("div", { className: "flex items-center" },
                        React.createElement("div", { className: "mr-2" }, "Search:"),
                        React.createElement("input", { type: "text", className: "w-full h-9 text-gray-800 dark:text-white border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 bg-gray-200 dark:bg-gray-700 px-4", value: this.state.search_term, onChange: this.handleSearchInputChange.bind(this) })),
                    React.createElement("div", null,
                        React.createElement(Select, { onChange: this.setSelectedFilterType.bind(this), options: (_a = this.props.custom_filter) !== null && _a !== void 0 ? _a : itemTypeFilter(), menuPosition: "absolute", menuPlacement: "bottom", styles: {
                                menuPortal: function (base) { return (__assign(__assign({}, base), { zIndex: 9999, color: "#000000" })); },
                            }, menuPortalTarget: document.body, value: this.getSelectedFilterValue() })),
                    React.createElement("div", null,
                        React.createElement(DangerOutlineButton, { button_label: "Clear Filters", on_click: this.clearFilters.bind(this) })))),
            React.createElement(Table, { columns: this.props.table_columns, data: this.state.items, dark_table: this.state.dark_tables })))));
    };
    return ItemTable;
}(React.Component));
