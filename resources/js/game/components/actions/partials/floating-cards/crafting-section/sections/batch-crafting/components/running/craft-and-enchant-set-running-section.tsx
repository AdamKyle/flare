...
const PHASE_LABELS: Record<CraftAndEnchantSetPhase, string> = {
  [CraftAndEnchantSetPhase.CRAFTING]: 'Crafting',
  [CraftAndEnchantSetPhase.ENCHANTING]: 'Enchanting',
};

const CraftAndEnchantSetRunningSection = ({
  batch,
  character_id,
  ...
  const phaseLabel = setProgress?.current_phase
    ? (PHASE_LABELS[setProgress.current_phase] ?? setProgress.current_phase)
    : '—';
    ...
    <div className="space-y-3">
      ...
      <BatchCraftingDetailSection title="Results">
        <dl className="xsm:grid-cols-2 grid grid-cols-1 gap-x-3 gap-y-1 text-sm">
          ...
          <dt className="text-gray-600 dark:text-gray-400">Current Phase</dt>
          <dd>{phaseLabel}</dd>
          ...
          <dt className="text-gray-600 dark:text-gray-400">Prefix / Suffix</dt>
          <dd>
...
export default CraftAndEnchantSetRunningSection;
