...
const TrinketryRunningSection = ({
  ...
  const renderCurrentItem = () => {
    ...
    if (
      progress.destination_set_id === null ||
      progress.destination_set_name === null
    ) {
      ...
      <LinkButton
        ...
        on_click={() =>
          openBatchCraftingSet(
            progress.destination_set_id as number,
            progress.destination_set_name as string,
            progress.current_item_name as string
          )
        }
        aria_label={`Open ${progress.current_item_name} in ${progress.destination_set_name}`}
  ...
  return (
    <div className="space-y-3">
      ...
      <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-sm">
        ...
        <dt className="text-gray-600 dark:text-gray-400">Copper Coins Spent</dt>
        <dd>{batch.copper_coins_spent.toLocaleString()}</dd>
        <dt className="text-gray-600 dark:text-gray-400">Copper Coins Left</dt>
