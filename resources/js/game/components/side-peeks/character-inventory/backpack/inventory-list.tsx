...
const InventoryList = ({items, is_quest_items, on_scroll_to_end}: InventoryListProps): ReactNode => {
  const renderBackPackItems = () => {
    ...
    if (isEmpty(items) && !is_quest_items) {
      return (
        <div className="text-center py-4">
          You have nothing in your inventory that you can equip. Either use the shop, craft or fight monsters to get some items.
          You can also checkout the market board for items as well that other players might be selling!
        </div>
      )
    }
  ...
  return (
    <div className="w-full h-full text-gray-800 dark:text-gray-200">
      <InfiniteScroll
          handle_scroll={on_scroll_to_end}
          additional_css={'my-4'}
        >
          {renderBackPackItems()}
        </InfiniteScroll>
