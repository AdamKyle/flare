...
export default class ItemComparison extends React.Component<ItemComparisonProps, ItemComparisonState> {
    ...
    renderEquipButtons(isInline: boolean, comparisonItemType?: ItemType): ReactNode {
        ...
        if (singleHandedItems.includes(itemType)) {
            return (
                <div className='flex justify-center'>
                    <PrimaryOutlineButton button_label={'Left Hand'} on_click={() => this.handleSecondaryAction('left-hand')} />
                    <PrimaryOutlineButton button_label={'Right Hand'} on_click={() => this.handleSecondaryAction('right-hand')} additional_css={'ml-4'} />
                </div>
            )
        }
        ...
        if (ItemType.RING === itemType) {
            return (
                <div className='flex justify-center'>
                    <PrimaryOutlineButton button_label={'Ring One'} on_click={() => this.handleSecondaryAction('ring-one')} />
                    <PrimaryOutlineButton button_label={'Ring Two'} on_click={() => this.handleSecondaryAction('ring-two')} additional_css={'ml-4'} />
                </div>
            )
        }
        ...
        if ([ItemType.SPELL_DAMAGE, ItemType.SPELL_HEALING].includes(itemType)) {
            return (
                <div className='flex justify-center'>
                    <PrimaryOutlineButton button_label={'Spell One'} on_click={() => this.handleSecondaryAction('spell-one')} />
                    <PrimaryOutlineButton button_label={'Spell Two'} on_click={() => this.handleSecondaryAction('spell-two')} additional_css={'ml-4'} />
                </div>
            )
        }
        ...
        return (
            <div className='flex justify-center'>
                <PrimaryOutlineButton button_label={'Equip'} on_click={() => this.handleSecondaryAction(this.props.comparison_info.itemToEquip.type)} />
    ...
    renderExpandedComparison() {
        ...
        return <ExpandedComparison comparison_details={this.state.expanded_comparison_details} mobile_data={{
        ...
        return (
            <div>
                <SuccessOutlineButton button_label={this.props.replace_button_text}
                                      on_click={() => this.handleSecondaryAction(this.props.comparison_info.details[0].position)}/>
            </div>
        )
    }
    ...
    shouldUseMobileHeightRestrictions(customWidth: number) {
        return this.state.view_port < customWidth && this.props.mobile_height_restriction
    }
        ...
        return (
            <div className='mr-auto ml-auto w-full md:w-3/5'>
                ...
                <div className="flex items-center">
                    ...
                    {this.renderSecondaryActionButton()}
                    {
                        nonArmourItems.includes(this.props.comparison_info.itemToEquip.type as ItemType) && this.shouldRenderPositionButtons() ?
                            <>
