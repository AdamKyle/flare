...
const baseModifierKeys = ["base_damage_mod", "base_ac_mod", "base_healing_mod"];
...
export default class AlchemyItemUsable extends React.Component<
    AlchemyItemUsableProps,
    {}
> {
    ...
    shouldRenderBaseSkillModifiers() {
        return (
            baseSkillModifierKeys.filter((key: string) => {
                return this.props.item[key] > 0;
            }).length > 0
        );
    }
    ...
    shouldRenderSkillModifiers() {
        return (
            miscSkillModifiers.filter((key: string) => {
                return this.props.item[key] > 0;
            }).length > 0
        );
    }
        ...
        return (
            <>
                <p className="mt-4 mb-4 text-sky-700 dark:text-sky-600">
                    This item can only be used on kingdoms. The damage it does
                    can stack if you drop multiple items. The damage can be
                    reduced by the kingdoms over all defence bonuses.
                </p>
                <p className="mb-4">
                    When you use this item on a kingdom you will do the % of
                    damage listed below for one item to all aspects of the
                    kingdom.
                </p>
                <dl>
    ...
    renderBaseSkillModifiers(): ReactNode {
        ...
        const modifiersList = validModifiers.map(
            (baseSkillModifier: string) => {
                return (
                    <>
                        <dt>
                        ...
                        <dd className={"text-green-700 dark:text-green-500"}>
                            +
                            {(this.props.item[baseSkillModifier] * 100).toFixed(
                                2,
                            )}
                            %
                        </dd>
        ...
        return (
            <>
                <p className={"my-4"}>
                    These modifiers apply to your class skill, which can be seen
                    on your character sheet, under skills. The skill will be
                    orange.
                </p>
                <dl>{modifiersList}</dl>
            </>
            ...
            (miscSkillModifier: string) => {
                return (
                    <>
                        ...
                        <dd className={"text-green-700 dark:text-green-500"}>
                            +
                            {(this.props.item[miscSkillModifier] * 100).toFixed(
                                2,
                            )}
                            %
                        </dd>
        ...
        return (
            <>
                <p className={"my-4"}>
                    ...
                    <strong>{this.props.item.skills.join(", ")}</strong>
                </p>
                <dl>{modifiersList}</dl>
            </>
        );
                ...
                <div>
                    {this.props.item.stat_increase ? (
                        <dl className={"my-4"}>
                            ...
                            <dd className="text-green-700 dark:text-green-500">
                                {(this.props.item.stat_increase * 100).toFixed(
                                    2,
                                )}
                                %
                            </dd>
