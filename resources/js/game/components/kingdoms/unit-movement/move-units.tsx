...
export default class MoveUnits {
    ...
    setAmountToMove(
        selectedUnits: SelectedUnitsToCallType[] | [],
        kingdomId: number,
        unitId: number,
        unitAmount: number,
        e: React.ChangeEvent<HTMLInputElement>,
        ...
        const index = unitsToCall.findIndex(
            (unitToCall: SelectedUnitsToCallType) => {
                return (
    ...
    getUnitOptions(
        ...
        selectedUnits: SelectedUnitsToCallType[] | [],
        selectedKingdoms: number[],
        setAmountToMove: setUnitAmount,
    ) {
        const kingdomsWithUnits = kingdoms.filter(
            (kingdom: KingdomReinforcementType) => {
        ...
        const units = kingdomsWithUnits.map(
            (kingdom: KingdomReinforcementType, kingdomIndex: number) => {
                return kingdom.units.map(
                    (unit: UnitReinforcementType, index: number) => {
                        return (
                            <div key={kingdom.kingdom_id + "-" + unit.id}>
                                {index === 0 ? (
                                    <p className="my-2">
                                        From Kingdom: {kingdom.kingdom_name} and
                                        will take:{" "}
                                        {self.getTimeToTravel(kingdom.time)} to
                                        get to this kingdom
                                    </p>
                                ) : null}
                                <div className="flex items-center my-4">
                                    <label className="w-1/2">
                                        {unit.name} Amount to move
                                    </label>
                                    <div className="w-1/2">
                                        <input
                                            type="number"
    ...
    getSelectedKingdomsValue(
        ...
        const foundKingdoms = selectedKingdoms.map((kingdom: number) => {
            const index = kingdoms.findIndex(
                (kingdomData: KingdomReinforcementType) => {
