...
> {
    ...
    setAmountToMove(
        ...
        const unitsToCall = this.moveUnits.setAmountToMove(
    ...
    setKingdoms(data: any) {
        const validData = data.filter(
            (data: any) => data.value !== "Please select one or more kingdoms",
        );

        let selectedKingdoms = JSON.parse(
            JSON.stringify(this.state.selected_kingdoms),
        );

        selectedKingdoms = validData.map(
            (value: any) => parseInt(value.value, 10) || 0,
        );
        ...
        return (
            <Fragment>
                {this.props.kingdoms.length > 0 ? (
                    ...
                    <InfoAlert>
                        You have no units in other kingdoms to move units from
                        or you have no other kingdoms.
                    </InfoAlert>
                )}
            </Fragment>
        );
