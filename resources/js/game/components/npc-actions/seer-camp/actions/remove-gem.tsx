...
export default class RemoveGem extends React.Component<any, RemoveGemsState> {
    ...
    componentDidMount() {
        (new Ajax()).setRoute('seer-camp/gems-to-remove/' + this.props.character_id).doAjaxCall('get', (result: AxiosResponse) => {
            this.setState({
    ...
    selectedItem() {
        ...
        const item = this.state.items.find((item: any) => item.slot_id === this.state.selected_item);
    ...
    getItemName(): string|null {
        const item: any = this.state.items.find((item: any) => item.slot_id === this.state.selected_item);
            ...
            <Fragment>
...

                {
                    this.state.selected_item !== 0 ?
                        <RemoveGemComparison comparison_data={this.state.selected_gem_data.comparison.atonement_changes}
                                             original_atonement={this.state.selected_gem_data.comparison.original_atonement}
