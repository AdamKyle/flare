...
export default class SmelterModal extends React.Component<any, any> {
    ...
    setAmount(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        this.setState({
            total: totalAmount,
            cost: totalAmount * 2,
            error_message: (totalAmount < 100 && totalAmount > 0) ? 'Can only smelt a minimum of 100 iron at a time.' : '',
            time_estimate: parseInt((totalTime - totalTime * this.props.smelting_time_reduction).toFixed(2)),
        });
    ...
    cancelSmelting() {
        this.setState({
            ...
            (new Ajax()).setRoute('kingdoms/cancel-smelting/' + this.props.kingdom_id)
                .doAjaxCall('post', (result: AxiosResponse) => {
                    this.setState({
                        loading: false,
                    }, () => {});
            ...
            <Fragment>
                ...
                <div className='my-4'>
                    <dl>
                        ...
                        <dt>Iron Cost:</dt>
                        <dd className={clsx({
                            'text-red-500 dark:text-red-400': this.state.cost > this.props.iron
                        })}>{formatNumber(this.state.cost)}</dd>
                        <dt>Time Estimate:</dt>
                        <dd>{formatNumber(this.state.time_estimate)} Minutes</dd>
                    </dl>
                </div>
                {
        ...
        return(
            <Dialogue is_open={this.props.is_open}
                      ...
                      secondary_actions={{
                          ...
                          secondary_button_disabled: this.state.total === 0 || this.state.total < 100 || this.props.iron === 0 || this.state.cost > this.props.iron || this.state.loading,
            ...
            >
                {
                    this.props.smelting_time_left > 0 ?
                        <Fragment>
                            <p className='my-4'>
                                Canceling this smelting request will only give you a percentage of iron back
                                based on the time elapsed.
                            </p>
                            <TimerProgressBar time_remaining={this.getTimeLeft()} time_out_label={'Smelting Steel: ' + formatNumber(this.props.smelting_amount)} />
                            <DangerButton button_label={'Stop Smelting'} on_click={this.cancelSmelting.bind(this)} additional_css={'my-4'} />
                            {
                                this.state.error_message !== '' ?
                                    <DangerAlert>{this.state.error_message}</DangerAlert>
                                    : null
