...
export const getPortLocation = (mapState: MapState): LocationDetails | null => {
    ...
    const portLocation = mapState.locations.filter(
        (location) =>
...
export const canSettleHere = (component: MapActions) => {
    ...
    if (component.props.locations !== null) {
        locations = component.props.locations.filter(
    ...
    const playerKingdom = component.props.player_kingdoms.filter(
    ...
    const enemyKingdoms = component.props.enemy_kingdoms.filter(
        (enemyKingdom: PlayerKingdomsDetails) => {
            return (
    ...
    const npcKingdoms = component.props.npc_kingdoms.filter(
        (npcKingdom: NpcKingdomsDetails) => {
            return (
