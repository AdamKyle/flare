...
export default class InventoryItemAttachedGems extends React.Component<any, any> {
    ...
    componentDidMount() {
        (new Ajax()).setRoute('socketed-gems/'+this.props.character_id+'/' + this.props.item_id)
        ...
        return this.state.attached_gems.map((gem: GemBagSlotDetails) => {
            return (
                <BasicCard additionalClasses='my-4'>
                    <h3 className='my-4 text-lime-600 dark:text-lime-500'>{gem.name}</h3>
                    <dl>
                        <dt>Tier</dt>
                        <dd>{gem.tier}</dd>
