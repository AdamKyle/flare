...
export default class InventoryItemGemDetails extends React.Component<any, any> {
    ...
    atonementChanges(originalAtonement: any, equippedAtonement: any): JSX.Element[]|[] {
        ...
        const atonements         = Object.entries(originalAtonement.atonements).map(([key, value]) => ({ [key]: value }));
        const equippedAtonements = Object.entries(equippedAtonement).map(([key, value]) => ({ [key]: value }));
        ...
        atonements.forEach((atonement: any) => {
            ...
            const value = this.findAtonementForChange(equippedAtonements, atonementName);
            ...
            if (total > value) {
                elements.push (
                    <Fragment>
                        <dt>{atonementName}</dt>
                        <dd className='text-green-700 dark:text-green-500'>+{(value === 0 ? total * 100 : (total - value) * 100).toFixed(2)}%</dd>
                    </Fragment>
                );
            }
            ...
            if (value < total) {
                elements.push (
                    <Fragment>
                        <dt>{atonementName}</dt>
                        <dd className='text-red-700 dark:text-red-500'>-{((total - value) * 100).toFixed(2)}%</dd>
                    </Fragment>
                );
            }
    ...
    findAtonementForChange(equippedAtonements: any, atonementName: string): number {
        let matchedValue = 0;
    ...
    render() {
        ...
        return (
            <div className={clsx({
                ...
                <div>
                    <BasicCard>
                        <RenderAtonementDetails title={'This Items Atonement'} original_atonement={this.props.item_atonement.atonements} />
