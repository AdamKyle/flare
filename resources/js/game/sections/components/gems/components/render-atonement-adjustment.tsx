...
export default class RenderAtonementAdjustment extends React.Component<
    any,
    any
> {
    ...
    renderDifference(
        ...
        return atonementKeys.map((atonementName: string) => {
            const atonementValue = this.findElementAtonement(
                originalAtonement,
                atonementName,
            );
    ...
    findElementAtonement(atonements: any, elementName: string): number {
        ...
        const element = atonementsKeys.filter(
            (atonementsName: string) => atonementsName === elementName,
        );
