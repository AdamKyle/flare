...
export default class ShopTableColumns {
    ...
    public buildColumns(
        ...
        if (typeof itemType === "undefined") {
            ...
            const isWeaponType =
                this.WEAPON_TYPES.filter((weaponType: ItemType) => {
                    return weaponType === itemType;
                }).length > 0;

            const isArmorType =
                this.ARMOUR_TYPES.filter((armorType: ItemType) => {
        ...
        shopColumns = [
            ...shopColumns,
            {
                name: "Actions",
                selector: (row: ItemDefinition) => row.name,
                cell: (row: ItemDefinition) => (
                    <div className={"my-2"}>
                        <div className="w-full mb-2">
                            <PrimaryButton
                                button_label={"Buy"}
                                on_click={() => this.buyItem(row)}
                        ...
                        <div className="w-full mb-2">
                            <PrimaryButton
                                ...
                                on_click={() =>
                                    this.viewComparison(row, viewComparison)
                                }
                                additional_css={"w-full"}
                            />
                        </div>
                        <div className="w-full">
                            <SuccessButton
                                ...
                                on_click={() =>
                                    this.viewPurchaseAny(row, viewBuyMany)
                                }
                                additional_css={"w-full"}
                            />
                        </div>
