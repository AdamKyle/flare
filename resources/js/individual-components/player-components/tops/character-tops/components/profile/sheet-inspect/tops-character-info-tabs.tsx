...
function FactionsList({ factions }: { factions: any }) {
    const list: any[] = (factions.factions ?? []).filter(
        (faction: any) =>
            (faction.current_level ?? 0) > 1 ||
            (faction.current_points ?? 0) > 0 ||
            faction.maxed === true,
    );
    const npcs: any[] = (factions.npcs ?? []).filter(
        (npc: any) =>
    ...
    return (
        <dl className="mt-2">
            {list.map((faction: any, index: number) => (
            ...
            {npcs.map((npc: any, index: number) => (
                <div key={`npc-${index}`}>
                    ...
                    <dd>
                        ...
                        {npc.currently_helping ? " — Currently helping" : ""}
                    </dd>
                    {(npc.next_level_fame ?? 0) > 0 ? (
                        <dd className="text-sm text-gray-600 dark:text-gray-400">
...
function PositiveNumberRow({ label, value }: { label: string; value: any }) {
    if (Number(value ?? 0) <= 0) {
        return null;
    }
...
export default function TopsCharacterInfoTabs({
    profile,
    manage_addition_data,
}: TopsCharacterInfoTabsProps) {
    const info = profile.info ?? profile.overview ?? {};
                        ...
                        <div>
                            <dl>
                                ...
                                <PositiveNumberRow
                                    ...
                                    value={info.heal_for}
                                />
                                <PositiveNumberRow label="AC" value={info.ac} />
                            </dl>
                        </div>
                    </div>
