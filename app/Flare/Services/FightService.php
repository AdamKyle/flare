...
class FightService {
    ...
    public function processFight($attacker, $defender, string $attackType) {
        $fightSetup    = $this->setupFightHandler->setUpFight($attacker, $defender);
        $newAttackType = $fightSetup->getAttackType();
        ...
        if (!is_null($newAttackType)) {
            ...
            if (count($attackType) > 1) {
                $attackType = $attackType[1];
            }
        }

        $this->battleLogs = $fightSetup->getBattleMessages();

        $newDefender = $fightSetup->getModifiedDefender();
        ...
        if (is_null($this->currentCharacterHealth) && is_null($this->currentMonsterHealth)) {
            ...
            $this->currentCharacterHealth = $characterInformation->buildHealth();
    ...
    public function fight($attacker, $defender, $attackType) {
        ...
        if ($attacker instanceof Character) {
            $data = $this->characterAttackHandler->setHealth(
                $this->currentCharacterHealth,
                $this->currentMonsterHealth,
            )->handleAttack($attacker, $defender, $attackType);

            $this->battleLogs             = [...$this->battleLogs, ...$data['battle_logs']];
            $this->currentMonsterHealth   = $data['current_monster_health'];
            $this->currentCharacterHealth = $data['current_character_health'];
        }
