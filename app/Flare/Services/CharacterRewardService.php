...
class CharacterRewardService
{
    ...
    {
        if ($amountPaid === RandomAffixDetails::COSMIC) {
            return $this->buildCosmicItem->fetchCosmicItem($this->character);
        }

        if ($amountPaid === RandomAffixDetails::MYTHIC) {
            return $this->buildMythicItem->fetchMythicItem($this->character);
        }

        return $this->buildUniqueItem->fetchUniqueItem($this->character);
    }
}
