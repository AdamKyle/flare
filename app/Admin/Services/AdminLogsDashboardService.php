...
class AdminLogsDashboardService
{
    ...
    public function entries(string $fileKey, int $page, string $severity, string $dateFrom, string $dateTo, int $perPage = 10): array
    {
        if (! isset(self::LOG_CHANNELS[$fileKey])) {
            return $this->pagination->transformCollectionResponse(collect(), $this->adminLogEntryTransformer, $perPage, 1);
        }
        ...
        return $this->pagination->transformCollectionResponse(
            collect($filtered),
            $this->adminLogEntryTransformer,
            $perPage,
            $page,
        );
    }
