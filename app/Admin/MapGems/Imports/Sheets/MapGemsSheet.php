...
class MapGemsSheet implements ToCollection
{
    ...
    private function normalizeRow(array $rawRow, string $name, int $rowNumber): array
    {
        ...
        $gameMap = GameMap::where('name', $gameMapName)->whereNull('generated_map_type')->first();

        if (is_null($gameMap)) {
            throw new RuntimeException('Row '.$rowNumber.': unknown or ineligible Game Map "'.$gameMapName.'".');
        }
        ...
        return [
            ...
            'character_power_reduction_range' => $this->resolveRange($rawRow['character_power_reduction_range'] ?? null, $rowNumber, 'character_power_reduction_range'),
