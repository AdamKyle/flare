...
class ExplorationTopsService
{
        ...
        $rows = ExplorationLog::with('character')->get()
            ...
            ->map(function ($logs) {
                ...
                return [
                    ...
                    'length_of_time_seconds' => (int) $logs->sum(fn ($log) => is_null($log->ended_at) ? 0 : $log->started_at->diffInSeconds($log->ended_at)),
    ...
    public function detail(ExplorationLog $explorationLog): array
    {
        ...
        return [
            ...
            'length_of_time_seconds' => is_null($explorationLog->ended_at) ? 0 : (int) $explorationLog->started_at->diffInSeconds($explorationLog->ended_at),
