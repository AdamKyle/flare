...
#[HandlesBatchCraftingType(BatchCraftingType::TRINKETRY)]
class TrinketryOrchestrator implements BatchCraftingOrchestrator
{
    public function __construct(private readonly BatchCraftingHandlerFactory $handlerFactory) {}

    /**
     * Resolve and run the Trinketry batch type's handler for the current mode.
     *
    ...
    public function orchestrate(BatchCrafting $batchCrafting, Character $character): BatchCraftingOperationResult
    {
        ...
        $mode = TrinketryBatchMode::from($progress['trinketry_mode']);

        $handler = $this->handlerFactory->make(BatchCraftingType::TRINKETRY, $mode);
