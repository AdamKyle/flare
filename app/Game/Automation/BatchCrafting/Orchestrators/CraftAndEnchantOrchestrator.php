...
#[HandlesBatchCraftingType(BatchCraftingType::CRAFT_AND_ENCHANT)]
class CraftAndEnchantOrchestrator implements BatchCraftingOrchestrator
{
    public function __construct(private readonly BatchCraftingHandlerFactory $handlerFactory) {}

    /**
     * Resolve and run the Craft and Enchant batch type's handler for the current mode.
     *
    ...
    public function orchestrate(BatchCrafting $batchCrafting, Character $character): BatchCraftingOperationResult
    {
        ...
        $mode = CraftAndEnchantBatchMode::from($progress['craft_enchant_mode']);

        $handler = $this->handlerFactory->make(BatchCraftingType::CRAFT_AND_ENCHANT, $mode);
