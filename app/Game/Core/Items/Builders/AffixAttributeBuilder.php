...
class AffixAttributeBuilder
{
    ...
    public function setBaseDetails(string $type, int $amountPaid): array
    {
        ...
        $names = match ($amountPaid) {
            RandomAffixTier::LEGENDARY->value => RandomAffixTier::legendaryNames(),
            RandomAffixTier::MYTHIC->value => RandomAffixTier::mythicalNames(),
            RandomAffixTier::COSMIC->value => RandomAffixTier::cosmicNames(),
            default => RandomAffixTier::legendaryNames(),
        };

        return [
            'name' => $names[$this->randomNumberGenerator->numberBetween(0, count($names) - 1)],
            'type' => $type,
    ...
    {
        return [
            'damage_amount' => $this->randomNumberGenerator->numberBetween($this->damageRange[0], $this->damageRange[1]) / 100,
    ...
    public function setSkillDetails(): array
    {
        ...
        return [
            'skill_name' => $this->characterSkills[$this->randomNumberGenerator->numberBetween(0, count($this->characterSkills) - 1)]->baseSkill->name,
    ...
    public function setSkillBonuses(): array
    {
        ...
        return [
            'affects_skill_type' => $allowedTypes[$this->randomNumberGenerator->numberBetween(0, count($allowedTypes) - 1)],
        ];
    }
    ...
    {
        $fraction = $this->randomNumberGenerator->numberBetween(0, 1_000_000) / 1_000_000;
