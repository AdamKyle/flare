...
class ItemTransferService
{
    ...
    /**
     * The currency costs to transfer item attributes.
     */
    public function getCosts(): array
    {
        return $this->currencyCosts;
    }
    ...
    public function transferItemEnhancements(Character $character, int $itemIdToTransferFrom, int $itemIdToTransferTo): array
    {
        ...
        if (is_null($itemSlotToTransferFrom) || is_null($itemSlotToTransferTo)) {
            return $this->errorResult('You do not have one of these items.');
        }

        if ($itemIdToTransferFrom === $itemIdToTransferTo) {
            return $this->errorResult('You cannot transfer attributes to the same item.');
        }
        ...
        $character->update([
            'gold' => $character->gold - $this->currencyCosts['gold'],
            'shards' => $character->shards - $this->currencyCosts['shards'],
            'gold_dust' => $character->gold_dust - $this->currencyCosts['gold_dust'],
        ]);
