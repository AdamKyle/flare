...
class AdventuresControllerTest extends TestCase
{
    ...
    public function testSaveAdventureFloorDescriptions() {
        ...
        $this->assertTrue($adventure->refresh()->floorDescriptions->isNotEmpty());
    }
    ...
    public function testFailToSaveFloorDescrtiptions() {
        ...
        $this->actingAs($this->user)->visit(route('adventures.list'))->visit(route('adventure.floor_descriptions', [
            'adventure' => $adventure->id
        ]))->see('Floor Descriptions for: ' . $adventure->name)
            ->submitForm('Save Floor Descriptions')
            ->see('Missing floor descriptions. Each floor needs a description.');
    }
}
