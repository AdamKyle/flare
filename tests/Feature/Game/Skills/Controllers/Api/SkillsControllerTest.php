...
class SkillsControllerTest extends TestCase
{
    ...
    public function testCanTrainDifferentSkill() {
        $user      = $this->character->getUser();
        $character = $this->character->trainSkill('Dodge')->getCharacter(false);
    ...
    public function testCannotCancelTrainForSkillYouDoNotOwn() {
        $otherUserSkill = (new CharacterFactory())->createBaseCharacter()->getCharacter(false)->skills()->first();
        $user      = $this->character->trainSkill('Dodge')
            ->updateSkill('Dodge', [
                'xp_towards' => 1
            ])
        ...
        $response = $this->actingAs($user)->json('post', '/api/skill/cancel-train/'.$user->character->id.'/' . $otherUserSkill->id)->response;
