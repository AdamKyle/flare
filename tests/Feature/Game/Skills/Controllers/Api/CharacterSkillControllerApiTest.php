...
class CharacterSkillControllerApiTest extends TestCase {
    ...
    public function testCannotCraftItemWhenCannotCraft() {
    ...
    public function testCanNotEnchantItemThatDoesntExistWithAffixThatDoesntExist() {
        ...
        $character = $this->character->assignSkill($skill)->getCharacter(false);
        $user      = $this->character->getUser();
    ...
    public function testFailedToCraft() {
        ...
        $craftingSkillService->shouldReceive('getDCCheck')->with($weaponCraftingSkill)->andReturn(100);
        $craftingSkillService->shouldReceive('characterRoll')->with($weaponCraftingSkill)->andReturn(1);
