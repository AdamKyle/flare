...
class KingdomUpdateServiceTest extends TestCase {
    ...
    public function testGiveKingdomToNPCWhenMoraleIsTooLowFromNotBeingWalked() {
        $kingdom = $this->createKingdomForCharacter();
        ...
        $kingdom->update([
            'current_morale'     => 0.05,
            'last_walked'        => now()->subDays(40),
        ]);

        $kingdom   = $kingdom->refresh();

        // Reduce Building durability for the morale to fall below 0.
        ...
        $this->assertNull($kingdom);

        $this->assertEquals(1, Kingdom::where('npc_owned', true)->count());
    }

    public function testGiveKingdomToNPCWhenMoraleIsTooLowFromBuildingDurability() {
        $kingdom = $this->createKingdomForCharacter();
