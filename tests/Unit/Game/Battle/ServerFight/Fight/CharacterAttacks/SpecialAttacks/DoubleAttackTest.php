...
class DoubleAttackTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $character = $this->specialAttackFactory->buildCharacter('Fighter', ['damage_stat' => 'str', 'to_hit_stat' => 'dex']);
        Cache::put('character-sheet-'.$character->id, [
            'level' => $character->level,
            'extra_action_chance' => ['has_item' => false, 'chance' => 1.0],
        ]);

        $special = $this->specialAttackFactory->buildDoubleAttack();
    ...
    public function test_double_attack_hits_twice_with_bonus_deduction_and_raid_cap(): void
    {
        $character = $this->specialAttackFactory->buildCharacter('Fighter', ['damage_stat' => 'str', 'to_hit_stat' => 'dex']);
        Cache::put('character-sheet-'.$character->id, [
            'level' => $character->level,
        ...
        $this->assertSame(
            BattleBase::MAX_DAMAGE_FOR_RAID_BOSSES * 100 - (int) (BattleBase::MAX_DAMAGE_FOR_RAID_BOSSES * 0.5) * 2,
            $special->getMonsterHealth()
        );

        $playerMessages = array_values(array_filter($special->getMessages(), fn ($msg) => $msg['type'] === 'player-action'));
        $this->assertCount(2, $playerMessages);
        $this->assertContains([
