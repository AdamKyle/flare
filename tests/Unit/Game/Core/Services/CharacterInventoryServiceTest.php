...
class CharacterInventoryServiceTest extends TestCase
{
    ...
    public function testInventoryIsNotEmpty() {
        $value = (new CharacterInventoryService)->setCharacter($this->character->getCharacter(false))->setInventory('weapon')->inventory();
    ...
    public function testInventoryIsNotEmptyWhenFetchingFromSetsWithPositions() {
        ...
        $character = $this->character->inventorySetManagement()->createInventorySets(10)->putItemInSet($this->item, 0, 'left-hand', true)->getCharacter(false);
