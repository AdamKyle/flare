...
class RandomItemDropBuilderTest extends TestCase
{
    ...
    public function test_generate_item_creates_item_with_prefix_only(): void
    {
        ...
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10)->andReturn(10);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10)->andReturn(10);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(0, 0)->andReturn(0);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10000)->andReturn(5001);
    ...
    public function test_generate_item_creates_item_with_prefix_and_suffix(): void
    {
        ...
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10)->andReturn(10);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(0, 0)->andReturn(0);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10000)->andReturn(5000);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(1, 10)->andReturn(10);
        $randomNumberGenerator->shouldReceive('numberBetween')->once()->with(0, 0)->andReturn(0);
        $builder = new RandomItemDropBuilder(
            $randomNumberGenerator,
