...
class CharacterInformationBuilderTest extends TestCase {
    ...
    public function testSumAllLifeStealingAffixes() {
        $character = $this->character->inventoryManagement()
            ...
            ->equipItem('left_hand', 'sample')
            ->equipItem('right_hand', 'sample 2')
            ->equipItem('spell_slot_one', 'sample 3')
            ->equipItem('spell_slot_one', 'sample 4')
            ->getCharacter(false);

        $amount = $this->characterInfo->setCharacter($character->refresh())->findLifeStealingAffixes(true);
